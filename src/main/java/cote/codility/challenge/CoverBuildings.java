...
public class CoverBuildings {
    ...
    public int getCoverSize(int[] H) {

        ArrayList<Integer> gab = new ArrayList<>();

        for (int i = 0; i < H.length - 1; i++) {
            gab.add(Math.abs(H[i] - H[i + 1]));
        }

        int maxGabIndex = 0;
        OptionalInt opInt = IntStream.range(0, gab.size())
                .reduce((a, b) -> gab.get(a) < gab.get(b) ? b : a);
        if (opInt.isPresent()) {
            maxGabIndex = opInt.getAsInt();
        }

        int x = 0;
        int y = 0;
        for (int i = 0; i <= maxGabIndex; i++) {
            x++;
            if (y < H[i]) {
                y = H[i];
            }
        ...
        for (int i = maxGabIndex + 1; i < H.length; i++) {
            x++;
            if (y < H[i]) {
                y = H[i];
            }
